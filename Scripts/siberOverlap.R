...
plot(dat.comp$d13C, dat.comp$Lat)

# But not as strong for d15N, although the shape of the relationship is similar
cor(dat.comp$d15N, dat.comp$Lat)
plot(dat.comp$d15N, dat.comp$Lat)

# Relationship is much weaker in total blood
cor(dat.comp$DL_d13C, dat.comp$Lat)
plot(dat.comp$DL_d13C, dat.comp$Lat)
cor(dat.comp$DL_d15N, dat.comp$Lat)
plot(dat.comp$DL_d15N, dat.comp$Lat)
...
mardiaTest(data = dat[dat$sex == "M" & dat$species == "NGP", c("d13C", "d15N")]) #fine
mardiaTest(data = dat[dat$sex == "F" & dat$species == "NGP", c("d13C", "d15N")]) #fine
mardiaTest(data = dat[dat$sex == "M" & dat$species == "SGP", c("d13C", "d15N")]) #fine
mardiaTest(data = dat[dat$sex == "F" & dat$species == "SGP", c("d13C", "d15N")]) #fine
...
shortmales <- unique(track[track$Maxdist < 70, "ID"])
