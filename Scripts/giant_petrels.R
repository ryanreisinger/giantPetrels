...
proj4string(mnt) <- crd
writeOGR(mnt, dsn = paste0("./Output/KML/Nests.kml"), layer = "Nests", driver = "KML")


# Sex
# Seems to be a break at ~96 mm
met$Culmen_Length <- as.numeric(met$Culmen_Length)
met$Culmen_Depth <- as.numeric(met$Culmen_Depth)

# Double check culmen-depth update for one individual
# Updated from meta in Data_original
...
# Molecular sexing results added 2017-07-11
...
# All known sexes match genetics results, so replace "Unknown"
...
# With ggmap
# zoom controls extent
m <- get_map(location = c(mean(dat$Longitude), mean(dat$Latitude)), zoom = 4, maptype = "satellite") 
ggmap(m) + geom_point(data = dat, aes(x = dat$Longitude, y = dat$Latitude, colour = Species))

# Sex
ggmap(m) + geom_point(data = dat, aes(x = dat$Longitude, y = dat$Latitude, colour = sex))

# All faceted
m <- get_map(location = c(mean(dat$Longitude), mean(dat$Latitude)), zoom = 4, maptype = "satellite") 
map <- ggmap(m, legend = "none")
map2 <- map + geom_point(data = dat, aes(x = dat$Longitude, y = dat$Latitude, colour = Species)) + facet_wrap(Species ~ sex)
map2
...
# Make sure sexes are updated if reading in existing RDS
tr[tr$ID == "NGP19_KD_SEP_2015", "sex"] <- "Male"
tr[tr$ID == "SGP02_KD_SEP_2015", "sex"] <- "Male"
...
crd <- CRS("+proj=longlat +ellps=WGS84 +datum=WGS84")
proj4string(mnt) <- crd
writeOGR(mnt, dsn = paste0("./Output/KML/", "NGP_males", ".kml"), layer = "NGP_males", driver = "KML")

mnt <- tr[tr$Species == "Southern giant petrel" & tr$sex == "Male", ]
coordinates(mnt) <- c("Longitude", "Latitude")
crd <- CRS("+proj=longlat +ellps=WGS84 +datum=WGS84")
proj4string(mnt) <- crd
writeOGR(mnt, dsn = paste0("./Output/KML/", "SGP_males", ".kml"), layer = "SGP_males", driver = "KML")
...
# Make sure sexes are updated if reading in existing RDS
tr2[tr2$ID == "NGP19_KD_SEP_2015", "sex"] <- "Male"
tr2[tr2$ID == "SGP02_KD_SEP_2015", "sex"] <- "Male"
...
trips <- unique(tr2$trip2)

# for (i in 1:length(trips)){
#   test <- tr2[tr2$trip2 == trips[i], ]
#   test$d <- as.numeric(test$date.time)
#   plot(test$date.time, test$Distance, pch = 19)
#   abline(h = 0.15)
#   t <- locator(2)$x
...
#   }
#   #test$d <- NULL
#   tr3 <- rbind.data.frame(tr3, test)
# }
# 
# tr3 <- tr3[tr3$trim2 == "Keep", ] #Remove the trimmed sections
# 
...
tr4sub <- tr4
disp <- ggplot(tr4sub, aes(x = lag/24, y = Distance, group = trip2, colour = sex)) +
  geom_line() +
  scale_colour_manual(values = c("#4daf4a", "#984ea3")) + 
  theme_bw() + theme(panel.grid.minor = element_blank(),
                              panel.grid.major = element_blank(),
                     axis.text = element_text(colour = "black")) +
  labs(x = "Days since deployment", y = "Distance from nest") #+
  # scale_x_continuous(limits = c(0, 17))
disp


# Short trips only
tr4short <- tr4[tr4$Maxdist < 30, ]
...
tr4sub.copy <- tr4sub
tr4sub.copy$which <- "All trips"
tr4sub.copy[tr4sub.copy$Maxdist < 30, "which"] <- "Short trips"
...
length(unique(tr4sub.copy[tr4sub.copy$sex == "Male" & tr4sub.copy$which == "All trips", "ID"]))
length(unique(tr4sub.copy[tr4sub.copy$sex == "Female" & tr4sub.copy$which == "All trips", "ID"]))
...
test2 <- unique(test2)
mean(test2[test2$Distance > 30 & test2$sex == "Male", "Culmen_Length"], na.rm = T)
mean(test2[test2$Distance < 30 & test2$sex == "Male", "Culmen_Length"], na.rm = T)
...
kud <- kernelUD(s, h = 1, grid = rt.sp) # Does not converge
image(kud)
...
# b4 <- as.raster(b3)
# b4[b4>0] <- NA # Set land
# writeRaster(b4, filename = "./Data/Mapping/bathymetry.grd", format = "raster")
...
#p1 <- p1 + scale_fill_distiller(palette = "Blues", direction = -1, name = "Depth")
...
# Crop raster
b5 <- getNOAA.bathy(37.45, 38.2, -47.2, -46.8, resolution = 0.1)
...
cowplot::plot_grid(p2, p3, axis = "t", ncol = 2, rel_widths = c(5, 3))
...
SST <- rbind.data.frame(SST1, SST2, SST3, SST4, SST5, SST6)
...
# Make sure sexes are updated if reading in RDS
tr3_env[tr3_env$ID == "NGP19_KD_SEP_2015", "sex"] <- "Male"
tr3_env[tr3_env$ID == "SGP02_KD_SEP_2015", "sex"] <- "Male"
